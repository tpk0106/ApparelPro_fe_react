...
const CurrencyConversionsTable = ({
  columns,
  data,
  setCurrencyConversionComponentPaginationState,
  itemsCount,
  ...
  const validateCurrencyConversion = ({ typeName }: CurrencyConversion) => {
    return {
      typeName: validationRequired(typeName) ? "Garment Type required" : "",
    };
  };
  ...
  const handleCreateCurrencyConversions: MRT_TableOptions<CurrencyConversion>["onCreatingRowSave"] =
    async ({ values, table }) => {
      values = {
        ...values,
        id: 0,
  ...
  //call CREATE hook
  const {
    mutateAsync: createCurrencyConversion,
    isPending: isCreatingCurrencyConversion,
  } = useCreateCurrencyConversion(pagination, paginate);
  ...
  //call UPDATE hook
  const {
    mutateAsync: updateCurrencyConversion,
    isPending: isUpdatingCurrencyConversion,
  } = useUpdateCurrencyConversion(pagination);
  ...
  const openDeleteConfirmModal = (row: MRT_Row<CurrencyConversion>) => {
    if (window.confirm("Are you sure you want to delete this Garment Type?")) {
      deleteCurrencyConversion(row.original.id);
    }
  };

  // call DELETE hook
  const {
    mutateAsync: deleteCurrencyConversion,
    isPending: isDeletingCurrencyConversion,
  } = useDeleteCurrencyConversion(pagination);
  //
  ...
  const table = useMaterialReactTable({
    ...
    onCreatingRowSave: handleCreateCurrencyConversions,
...
export default CurrencyConversionsTable;
