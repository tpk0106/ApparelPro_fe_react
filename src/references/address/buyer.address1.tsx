...
const BuyerAddresses = ({
  ...
  const buyerAddresses = useMemo(() => {
    const addresses: Address[] = [];
    return addresses;
  }, []);

  // function GetCountries() {
  //   const { data } = useCountries(paginate);
  //   return data;
  // }
  ...
  //const countries1 = useMemo(()=>useSelector(selectAllCountryCodes), []);
  ...
  const { mutateAsync: createBuyerAddress, isPending: isCreatingBuyerAddress } =
    useCreateBuyerAddress(pagination);
  ...
  const { mutateAsync: deleteBuyerAddress, isPending: isDeletingCountry } =
    useDeleteBuyerAddress(pagination);
  ...
  const table = useMaterialReactTable({
    ...
    //muiPaginationProps: { rowsPerPageOptions: [5, 10] },
